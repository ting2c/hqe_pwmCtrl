...
uint32_t   GulNum;
uint8_t 	 bUART0_RecvFIFO[RecvSize];
uint8_t 	 bUART0_TranFIFO[(RecvSize << 1)];
uint32_t 		usr_duty = 1;
uint32_t 		opt = 0;
uint32_t 		quick_fix = 0;
uint32_t 		quick_fix1 = 0;
uint64_t 		_10us_cnt = 0;
	...
	while ((II_Buf & mskUART_II_STATUS) == UART_II_STATUS)		//check interrupt status, the INT can be determined by USARTn_II[3:1]
	{
      ...
      case UART_RDA:		//Receive Data Available
				...
				if(GulNum == RecvSize)
				{
					...
					if(bUART0_RecvNew == 1)
					{
						switch(bUART0_RecvFIFO[0])
						{
							case 0xF5:
								if(bUART0_RecvFIFO[1] == bUART0_RecvFIFO[2])
								{
									if(bUART0_RecvFIFO[2] <= 101)
									{
										usr_duty = (bUART0_RecvFIFO[2] - 1);
...
void UART0_Init (void)
{
		...
		*/
		...
		SN_UART0->FD = (UART_OVER_SAMPLE_16|UART_MULVAL_7|UART_DIVADDVAL_0);
		SN_UART0->DLM = 0;
		SN_UART0->DLL = 78;
		...
		// FIFO Control
		SN_UART0->FIFOCTRL =(
												UART_FIFO_ENABLE // Enable USART FIFOs
